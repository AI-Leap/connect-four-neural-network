class Connect4:
    ...
    def getMoves(self):
        ...
        for i in range(self.numberOfColumns):
          if self.isAvailable(self.board[0], i):
            moves.append(i)
        return moves
    ...
    def player_choice(self):
        ...
        while self.board[0][choice] != '  ':
            choice = int(
                input("This column is full. Please choose between 0 and 6 : "))
        return choice
    ...
    def getWinner(self):
        xWin = self.checkLines('X') or self.checkLines('X', self.generateReversedBoard()) or self.checkDiags('X')
        oWin = self.checkLines('O') or self.checkLines('O', self.generateReversedBoard()) or self.checkDiags('O')
